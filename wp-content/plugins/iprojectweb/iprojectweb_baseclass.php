 ...
 * @file
 *
 * 	iProjectWebBase class definition
 */
...
class iProjectWebBase {
	...
	function getNew() {
		...
		$insid = iProjectWebDB::insert($this->fieldmap, $this->type);
		$fieldlist = $this->getFieldNames();
		if (in_array('ListPosition', $fieldlist)) {
			iProjectWebDB::updateListPosition($this->type, $insid);
		}
	...
	function update($request, $id) {
		...
		iProjectWebDB::update($fldvalues, $this->type, $id);
	...
	function getTableName() {
		...
		return iProjectWebDB::getTableName($this->type);
	...
	function getFilter($viewmap, $rparams = NULL) {
		...
		if (isset($viewmap['filter'])) {
			...
			$rparams = iProjectWebDB::getStdFilters($jspar, $rparams);
		}
	...
	function getOrder($viewmap) {
		...
		if (isset($viewmap['sort'])) {
			...
			$spar = iProjectWebDB::getStdOrder($spar);
		}
		return $spar;
	...
	function getStdList($obj, $fld = NULL) {
		...
		$tablename = iProjectWebDB::getTableName($obj);
	...
	function getList($listquery, $params = NULL) {
		...
		$listquery = iProjectWebDB::getQueryText($listquery, $params);
		if (!isset($this->rsholder[$listquery])) {
			$rslist = iProjectWebDB::getObjects($listquery);
	...
	function getRoleListHTML($fieldname, $notnull = FALSE) {
		...
		$rslist = $this->getList(
			'SELECT id, Description FROM #wp__iprojectweb_roles WHERE id <> 2'
		);
	...
	function getBasicASList($_asmap, $query) {
		...
		$_items = iProjectWebDB::select($query, $values);
	...
	function getJSConfig($map) {
		...
		return	iProjectWebUtils::toJs($config);
	...
	function sList($group, $useany = TRUE) {
		...
		return iProjectWebDB::getSignSList($group, $useany);		
	...
	function dispatch($dispmap) {
		...
		$dispmap = iProjectWebUtils::intercept($dispmap);
		if ($dispmap == NULL) {
			iProjectWebIHTML::getNotLoggedInHTML();
			return NULL;
		}
		$this->user = $dispmap['iprojectusr'];
		$this->map = $dispmap;
