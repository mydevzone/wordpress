 ...
 * @file
 *
 * 	iProjectWebRiskImpacts class definition
 */
 ...
 *
 */
class iProjectWebRiskImpacts extends iProjectWebBase {

	/**
	 * 	iProjectWebRiskImpacts class constructor
	 *
	 ...
	 * 	getDeleteStatements
	 *
	 * 	prepares delete statements to be executed to delete a riskimpact
	 * 	record
	 *
	 * @param int $id
	 * 	object id
	...
	function getDeleteStatements($id) {
		...
		$stmts[] = "DELETE FROM #wp__iprojectweb_riskimpacts WHERE id=$id;";
	...
	function getMainForm($formmap) {
		...
		require_once 'views/iprojectweb_riskimpactsmainform.php';
	...
	function getMainView($viewmap) {
		...
		$orderby = iProjectWebDB::getOrderBy(array('id', 'Description', 'ListPosition'), $spar, "RiskImpacts.ListPosition");

		$rparams = $this->getFilter($viewmap);
		$viewfilters = array();
		$viewfilters = iProjectWebDB::getSignFilter($viewfilters, $rparams, 'RiskImpacts.', 'id', 'int');
		$viewfilters = iProjectWebDB::getSignFilter($viewfilters, $rparams, 'RiskImpacts.', 'Description');
		$viewfilters = iProjectWebDB::getSignFilter($viewfilters, $rparams, 'RiskImpacts.', 'Notes');

		iProjectWebRoot::moveRow('RiskImpacts', $viewmap, $viewfilters, 'RiskImpacts');
		iProjectWebRoot::mDelete('RiskImpacts', $viewmap);

		$query = "SELECT
				RiskImpacts.id,
				RiskImpacts.Description,
				RiskImpacts.ListPosition
			FROM
		...
		$obj = $this;
		?><input type='hidden' name='t' id='t' value='RiskImpacts'><?php

		require_once 'views/iprojectweb_riskimpactsmainview.php';
	...
	function getReadonlyView($viewmap) {
		...
		require_once 'views/iprojectweb_riskimpactsreadonlyview.php';
