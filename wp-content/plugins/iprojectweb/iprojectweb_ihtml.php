 ...
 * @file
 *
 * 	iProjectWebIHTML class definition
 */
...
class iProjectWebIHTML {
	...
	function getScroller($obj) {
		...
		<div class = 'scrollerpanel'>
			...
			<div>
				<span class = 'label'>
					<?php echo IPROJECTWEB_ScrollerRows;?>
				</span>
			</div>
			<?php if ($showlist) { ?>
				<div>
					<select
						...
						id = 'limit'
						onchange = 'filter(<?php echo $obj->jsconfig;?>)'>
						<?php echo iProjectWebIHTML::getScrollerStepsList($obj->limit, $options); ?>
					</select>
				</div>
			<?php }
						else { ?>
			...
			<div style = 'white-space:nowrap;'>
				<span class = 'label'>
					(<?php echo ($obj->start + 1); ?>-<?php echo $count . ' ' . IPROJECTWEB_Of . ' ' . $obj->rowCount;?>)
				</span>
			</div>
			<div>
				...
				<?php echo iProjectWebIHTML::getButton(
					array(
						'title' => IPROJECTWEB_ScrollerFirst,
						'events' => ' onclick = \'scroll(' . $obj->jsconfig . ', -2)\'',
			...
			<div>
				...
				<?php echo iProjectWebIHTML::getButton(
					array(
						'title' => IPROJECTWEB_ScrollerBack,
						'events' => ' onclick = \'scroll(' . $obj->jsconfig . ', -1)\'',
						'bclass' => 'ufo-imagebutton',
			...
			<div>
				...
				<?php echo iProjectWebIHTML::getButton(
					array(
						'title' => IPROJECTWEB_ScrollerForward,
						'events' => ' onclick = \'scroll(' . $obj->jsconfig . ', 1)\'',
						'bclass' => 'ufo-imagebutton',
			...
			<div>
				...
				<?php echo iProjectWebIHTML::getButton(
					array(
						'title' => IPROJECTWEB_ScrollerLast,
						'events' => ' onclick = \'scroll(' . $obj->jsconfig . ', 2)\'',
						'bclass' => 'ufo-imagebutton',
		...
		<div class = 'listpositionmover'>
			<div class = 'commonbutton'>
				<div
					...
					onclick = 'moveRow(<?php echo $p->jsconfig;?>, 1,"<?php echo $p->id;?>")'>
				</div>
			</div>
			<div class = 'commonbutton'>
				<div
	...
	function getTinyMCE($id) {
		...
		$tmceconf = iProjectWebApplicationSettings::getInstance()->get('TinyMCEConfig');
		?>
		<input
			type = 'hidden'
			class = 'ufo-eval'
	...
	function getFileUpload($id, $object) {
		...
		$jsrequest = iProjectWebUtils::toJs($object->request);
		$jsoncomplete = isset($object->oncomplete) ?
			', function(file, ext) {' . $object->oncomplete->func . '(' . $object->oncomplete->args . ')}'
			: '';
		$label = isset($object->value) ? IPROJECTWEB_Update : IPROJECTWEB_Upload;
		?>
		<input
			type = 'hidden'
			class = 'ufo-eval'
		...
		<span
			...
			onclick = 'deleteFile("<?php echo $id;?>",<?php echo $jsrequest;?>, this<?php echo $jsoncomplete;?>);'>
			<span>
				<a
					class = 'icon_button_delete'>
					<?php echo IPROJECTWEB_Delete;?>
				</a>
			</span>
		</span>
		<?php
	...
	function getFileDownloadLink($config) {
		...
		require_once 'iprojectweb_files.php';

		$url = iProjectWebFiles::getFileDownloadLink(
			$config->doctype,
			$config->field,
			$config->docid
		);
	...
	function getColumnHeader($params) {
		...
		$view = $p->view;
		$id = $view->oId($p->field);
		$label = isset($p->label) ? $p->label : constant('IPROJECTWEB_' . $p->field);
		if (isset($view->map['r'])) {
			echo $label;
			return;
		}
		?>
			<button type = 'button' class = 'ufo-tableheader thnoorder'
				id = '<?php echo $id;?>'
				onclick = 'sort(<?php echo $view->jsconfig;?>, "<?php echo $p->field;?>"); return false;'>
				<span class = 'thtext'>
					<?php echo $label;?>
				</span>
	...
	function getNotLoggedInHTML() {
		...
		echo iProjectWebApplicationSettings::getInstance()->get('NotLoggenInText');
