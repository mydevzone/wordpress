 ...
 * @file
 *
 * 	iProjectWebProjectStatuses class definition
 */
 ...
 *
 */
class iProjectWebProjectStatuses extends iProjectWebBase {

	/**
	 * 	iProjectWebProjectStatuses class constructor
	 *
	 ...
	 * 	getDeleteStatements
	 *
	 * 	prepares delete statements to be executed to delete a projectstatus
	 * 	record
	 *
	 * @param int $id
	 * 	object id
	...
	function getDeleteStatements($id) {
		...
		$stmts[] = "DELETE FROM #wp__iprojectweb_projectstatuses WHERE id=$id;";
	...
	function getMainForm($formmap) {
		...
		require_once 'views/iprojectweb_projectstatusesmainform.php';
	...
	function getMainView($viewmap) {
		...
		$orderby = iProjectWebDB::getOrderBy(array('id', 'Description', 'ListPosition'), $spar, "ProjectStatuses.ListPosition");

		$rparams = $this->getFilter($viewmap);
		$viewfilters = array();
		$viewfilters = iProjectWebDB::getSignFilter($viewfilters, $rparams, 'ProjectStatuses.', 'id', 'int');
		$viewfilters = iProjectWebDB::getSignFilter($viewfilters, $rparams, 'ProjectStatuses.', 'Description');
		$viewfilters = iProjectWebDB::getSignFilter($viewfilters, $rparams, 'ProjectStatuses.', 'Notes');

		iProjectWebRoot::moveRow('ProjectStatuses', $viewmap, $viewfilters, 'ProjectStatuses');
		iProjectWebRoot::mDelete('ProjectStatuses', $viewmap);

		$query = "SELECT
				ProjectStatuses.id,
				ProjectStatuses.Description,
				ProjectStatuses.ListPosition
			FROM
		...
		$obj = $this;
		?><input type='hidden' name='t' id='t' value='ProjectStatuses'><?php

		require_once 'views/iprojectweb_projectstatusesmainview.php';
	...
	function getReadonlyView($viewmap) {
		...
		require_once 'views/iprojectweb_projectstatusesreadonlyview.php';
